...
def get_existence_probability(x, max_x, bound):
    first_point = 0.9
    last_point = 0.1
    
    upper_bound = first_point + (x - 1) * ((last_point - first_point) / (max_x - 1))
...
def get_movenorth_action(width, height, bound):
    action = []
    ...
    for x in xrange(1, width + 1):
        ...
        action.append( (from_state, to_state, 1.0, 1.0) )
        ...
        for y in xrange(1, height - 1):
            from_state = get_state_string(x, y)
            to_state = get_state_string(x, y + 1)
            lower_probability, upper_probability = get_existence_probability(x, width, bound)
            
            action.append( (from_state, to_state, lower_probability, upper_probability) )
            action.append( (from_state, missing_robot_state, 1.0 - upper_probability, 1.0 - lower_probability) )

    action.append( (missing_robot_state, missing_robot_state, 1.0, 1.0) )
    return action

def get_movesouth_action(width, height, bound):
    action = []
...
def get_moveeast_action(width, height, bound):
    action = []
    ...
    for y in xrange(1, height + 1):
        ...
        for x in xrange(1, width):
            ...
            if y == 1 or y == height:
                ...
                #third case, another cells
                lower_probability, upper_probability = get_existence_probability(x + 1, width, bound)
...
def get_movewest_action(width, height, bound):
    action = []
        ...
        for x in xrange(2, width + 1):
            ...
            if x == width and y == height:
                ...
                if y == 1 or y == height:
                    ...
                    #third case, another cells
                    probability = get_existence_probability(x - 1, width, bound)
...
def get_actions(width, height, bound):
    actions = {}

    actions["move-north"] = get_movenorth_action(width, height, bound)
    actions["move-south"] = get_movesouth_action(width, height, bound)
    actions["move-east"] = get_moveeast_action(width, height, bound)
    actions["move-west"] = get_movewest_action(width, height, bound)
...
def generate_domain_data(width, height, bound):
    states = get_states(width, height)

    actions = get_actions(width, height, bound)
...
if __name__ == "__main__":
    ...
    domain_data = generate_domain_data(args.width, args.height, args.bound)
